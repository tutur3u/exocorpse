...
-- ============================================================================
-- Note: These are basic policies. You may want to adjust them based on your
...
CREATE POLICY "Public stories are viewable by everyone"
  ...
  USING (visibility = 'public' AND is_published = true AND deleted_at IS NULL);
...
CREATE POLICY "Users can soft delete their own stories"
  ON stories FOR DELETE
  USING (auth.uid() = created_by);

-- Worlds policies (inherit from story visibility)
CREATE POLICY "Worlds are viewable if their story is viewable"
...
CREATE POLICY "Users can create worlds in their stories"
...
-- Characters policies (inherit from world/story visibility)
CREATE POLICY "Characters are viewable if their world/story is viewable"
...
-- (You can expand these as needed for factions, locations, events, etc.)
