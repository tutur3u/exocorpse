...
export default function CmsAssetManager({
  ...
  const partitionedAssets = partitionCmsAssets(sortedAssets);
  const visibleAssets =
    mode === "single"
    ...
    <section className="space-y-4">
      {mode === "gallery" ? (
        <div
          ...
          className="inline-flex max-w-full rounded-lg border border-slate-300 bg-slate-100 p-1 dark:border-slate-700 dark:bg-slate-950/70"
          role="tablist"
        >
          {(
            [
              ["linked", "Linked media", partitionedAssets.linked.length],
              ["inline", "Inline media", partitionedAssets.inline.length],
            ] as const
          ).map(([tab, label, count]) => (
            <button
              aria-selected={activeMediaTab === tab}
              className="rounded-md px-3 py-1.5 text-xs font-semibold text-slate-500 transition hover:text-slate-900 aria-selected:bg-white aria-selected:text-slate-950 aria-selected:shadow-sm dark:text-slate-400 dark:hover:text-slate-100 dark:aria-selected:bg-slate-800 dark:aria-selected:text-white"
              key={tab}
      ...
      {showUpload ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-950/40 dark:text-slate-300">
          Inline images are inserted inside text. They are shown here for
          reference and can be removed without mixing them into the public
          gallery.
        </div>
      )}
        ...
        {(asset) => {
          ...
          return (
            <article
              ...
              key={asset.id}
            >
              {isVisualCmsAsset(asset) && imageUrl ? (
                <div
              ...
              <div className="flex items-center gap-3 p-3">
                <div className="min-w-0 flex-1">
                  ...
                  <Badge
                    className="mt-1 border-slate-300 bg-transparent text-[0.62rem] text-slate-500 dark:border-slate-700 dark:text-slate-300"
                    variant="outline"
                  >
                    {cmsAssetTypeLabel(asset)}
                  </Badge>
                </div>
                <Button
      ...
      {visibleAssets.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-gray-300 px-4 py-8 text-center text-sm text-gray-500 dark:border-gray-600 dark:text-gray-400">
          {mode === "single"
            ...
            : activeMediaTab === "inline"
              ? "No inline images yet. Add one from a text editor's image tool."
              : "No linked media yet. Choose or drop a file to add the first one."}
        </div>
      ) : null}
    </section>
  );
