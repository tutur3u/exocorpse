...
import { updateLocationGalleryItem } from "@/lib/actions/wiki";
import { cleanFormData } from "@/lib/forms";
import { useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import type { Tables } from "../../../../supabase/types";

type LocationGalleryImage = Tables<"locations_gallery_images">;
...
type LocationGalleryItemFormProps = {
  locationId: string;
  galleryItem?: LocationGalleryImage;
  onSubmit: (
    data: GalleryItemSubmitData,
  ) => Promise<LocationGalleryImage | void>;
  onComplete: () => void;
  onCancel: () => void;
};

export default function LocationGalleryItemForm({
  locationId,
  galleryItem,
  onSubmit,
  onComplete,
  onCancel,
}: LocationGalleryItemFormProps) {
  const {
    setPendingFile,
    uploadPendingFiles,
    uploadProgress,
    ...
    try {
      ...
      if (result && hasPendingFiles) {
        const uploadSuccess = await uploadPendingFiles(
          ...
          async (updates) => {
            await updateLocationGalleryItem(result.id, updates);
          },
        );
  ...
  const handleDeleteOldImage = async (oldPath: string | undefined) => {
    ...
    if (
      ...
      oldPath.includes("locations/")
    ) {
      try {
        await deleteLocationGalleryImage(oldPath);
      } catch (err) {
            ...
            <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
              ...
              <div>
                <ImageUploader
                  ...
                  uploadPath={
                    locationId ? `locations/${locationId}/gallery` : undefined
                  }
              ...
              <div>
                ...
                <input
                  ...
                  {...register("tags")}
                  className="w-full rounded-md border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                  placeholder="landscape, map, aerial (comma separated)"
                />
