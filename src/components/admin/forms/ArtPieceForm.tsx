...
import { updateArtPiece } from "@/lib/actions/portfolio";
...
type ArtPieceFormProps = {
  artPiece?: ArtPiece;
  onSubmit: (data: ArtPieceSubmitData) => Promise<ArtPiece | void>;
  onComplete: () => void;
  onCancel: () => void;
};
...
export default function ArtPieceForm({
  ...
  const handleFormSubmit = formHandleSubmit(async (data) => {
    ...
    try {
      ...
      const submitData: ArtPieceSubmitData = {
        ...cleanData,
        tags: tagsArray,
      };

      // Submit the artwork data
      ...
      if (result && hasPendingFiles) {
        const uploadSuccess = await uploadPendingFiles(
          result.id,
          `portfolio/art/${result.id}`,
          async (updates) => {
            await updateArtPiece(result.id, updates);
          },
        );
